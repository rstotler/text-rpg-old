...
class Action:
    ...
    def update(self, console, player):
        self.currentTick += 1
        if self.currentTick >= self.maxTick:
            if self.actionType == "Reload":
                self.completeReload(console, player)
            elif self.actionType == "Unload":
                self.completeUnload(console, player)

    def completeReload(self, console, player):
        magazineCheck = False
        reloadCount = 0
        reloadWeapon = None
        requireWeapon = None
        ...
        for weaponIndex, weapon in enumerate(reloadList):
            ...
            if weapon.shellCapacity != None:
                targetAmmo, ammoIndex = player.ammoCheck(weapon.ammoType)
                if ammoKey != None:
                    targetAmmo, ammoIndex = player.ammoCheck(weapon.ammoType, ammoKey)
            ...
            # Magazine Weapons #
                ...
                targetMagazine, magazineIndex = player.ammoCheck(weapon.ammoType, None, True)
                if magazineIndex != -1:
                    magazineCheck = True
                else:
                    ...
                    if ammoIndex != -1 and not (reloadKey == "All" and ammoKey == None and weapon.magazine != None and weapon.magazine.flags["Ammo"] != None and weapon.magazine.flags["Ammo"].num != targetAmmo.num):
                        if weapon.magazine == None or weapon.magazine.flags["Ammo"] == None or weapon.magazine.flags["Ammo"].quantity < weapon.magazine.shellCapacity or weapon.isLoaded(player.itemDict["Ammo"]) == False or (targetAmmo != None and weapon.magazine != None and weapon.magazine.flags["Ammo"] != None and targetAmmo.num != weapon.magazine.flags["Ammo"].num):
                            ...
                            if weaponMagazine == None:
                                ...
                                player.itemDict["Ammo"].append(weaponMagazine)
                                if weaponMagazine.flags["Ammo"] != None:
                                    heldAmmo, unused = player.getTargetItem(weaponMagazine.flags["Ammo"].num, ["Inventory"])
                                    if heldAmmo != None:
                                        heldAmmo.quantity += weaponMagazine.flags["Ammo"].quantity
                                    else:
                                        player.itemDict["Ammo"].append(weaponMagazine.flags["Ammo"])
                            ...
                            if weapon.magazine != None and weapon.magazine.flags["Ammo"] != None and weapon.magazine.flags["Ammo"].num != targetAmmo.num:
                                ...
                                if inventoryAmmo != None:
                                    ...
                                    player.itemDict["Ammo"].append(weapon.magazine.flags["Ammo"])
            ...
            if targetAmmo != None:
                if reloadQuantity >= targetAmmo.quantity:
                    del player.itemDict["Ammo"][ammoIndex]
                else:
                    targetAmmo.quantity -= reloadQuantity
            if targetMagazine != None and ammoCheck == True and heldMagazineCheck == True:
                if targetMagazine in player.itemDict["Ammo"]:
                    magazineIndex = player.itemDict["Ammo"].index(targetMagazine)
                    del player.itemDict["Ammo"][magazineIndex]
    ...
    def completeUnload(self, console, player):
        unloadWeapon = None
        unloadCount = 0
        targetAmmo = None
        targetMagazine = None
